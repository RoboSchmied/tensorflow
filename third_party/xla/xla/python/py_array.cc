...
bool IsZeroCopyableCpuBuffer(const PjRtBuffer* buf) {
  ...
  // to unpack the array. This could happen for the host buffer pre-mapped
  // to the TPU device, a.k.a., pinned host buffers for the device.
  bool has_default_layout = buf->on_device_shape().has_layout() &&
                            HasDefaultLayout(buf->on_device_shape().layout());
