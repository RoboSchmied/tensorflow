...
class RewriteQuantizedDynamicSliceOp
 ...
 public:
  ...
  LogicalResult match(stablehlo::DynamicSliceOp op) const override {
    ...
    return success(operand_type.hasStaticShape());
  }
  ...
  void rewrite(stablehlo::DynamicSliceOp op,
    ...
    // For all start indices, cast tensor<i64> to tensor<1xi64> and clamp with
    // upper limits.
    ...
    ArrayRef<int64_t> size_len(rank);
