 ...
 private:
  class Iterator : public DatasetIterator<Dataset> {
   public:
    ...
    // TODO(b/325112575): Move this to the base class so that any class with a
    // global shuffle iterator gets this behavior automatically.
    absl::Status SkipInternal(IteratorContext* ctx, int num_to_skip,
                              bool* end_of_sequence,
                              int* num_skipped) override {
      if (ctx->index_mapper() == nullptr) {
        return DatasetIterator::SkipInternal(ctx, num_to_skip, end_of_sequence,
                                             num_skipped);
      }
      global_shuffle_iterator_.Skip(num_to_skip, num_skipped);
      return absl::OkStatus();
    }
